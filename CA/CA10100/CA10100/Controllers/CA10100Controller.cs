...
using System.IO;
using System.Text;
namespace CA10100.Controllers
{
    [DirectController]
    [CustomAuthorize]
    [CheckSessionOut]
    public class CA10100Controller : Controller
    {
        private string _screenNbr = "CA10100";
        private string _userName = Current.UserName;

        CA10100Entities _db = Util.CreateObjectContext<CA10100Entities>(false);
        ...
        {
            var obj = _db.CA10100_pdHeader(Current.CpnyID, Current.UserName, Current.LangID, BranchID,BatNbr).FirstOrDefault();
            return this.Store(obj);
        }
        public ActionResult GetDetail(string BranchID, string BatNbr)
        {
            var lstobj = _db.CA10100_pgDetail(Current.CpnyID, Current.UserName, Current.LangID, BranchID, BatNbr).ToList();           
        ...
        public ActionResult Save(FormCollection data)
        {
            try
            {
                ...
                var curHeader = dataHandler.ObjectData<CA10100_pdHeader_Result>().FirstOrDefault();
                ...
                ChangeRecords<CA10100_pgDetail_Result> lstDetail = dataHandler1.BatchObjectData<CA10100_pgDetail_Result>();
                #region Save Header
                ...
                else
                {
                    ...
                    header.RefNbr = curHeader.RefNbr.PassNull() != "" ? curHeader.RefNbr : (curHeader.Type == "R" ? _db.CANumbering(BranchID, "ReceiptNbr").FirstOrDefault() : _db.CANumbering(BranchID, "PaymentNbr").FirstOrDefault());
                    header.JrnlType = "CA";
                ...
                #region Save Detail
                foreach (CA10100_pgDetail_Result deleted in lstDetail.Deleted)
                {
                    ...
                    {
                        var objDelete = _db.CA_Trans.FirstOrDefault(p => p.BranchID == BranchID && p.BatNbr == BatNbr && p.RefNbr == header.RefNbr && p.LineRef == deleted.LineRef.ToUpper());
                        if (objDelete != null)
                        {
                ...
                foreach (CA10100_pgDetail_Result curRow in lstDetail.Created)
                {
                    if (curRow.EntryID.PassNull() == "") continue;
                ...
                else if (Handle == "V" || Handle == "C")
                {
                    ...
                    if (Handle == "C") //copy thành lô mới
                    {
                        ...
                        var lstRowDB = _db.CA_Trans.Where(p => p.BranchID == BranchID && p.BatNbr == header.BatNbr && p.RefNbr == header.RefNbr).ToList();
        ...
        private void UpdatingHeader(ref Batch t, CA10100_pdHeader_Result s)
        {


            t.RvdBatNbr = s.RvdBatNbr;
            t.ReasonCD = s.BankAcct;          
            t.TotAmt = s.TotAmt;
            t.DateEnt = s.DateEnt;
            t.EditScrnNbr = "CA10100";
            t.Module = "CA";
            t.JrnlType = t.JrnlType.Length == 0 ? "CA" : t.JrnlType;
            t.Status = s.Status;
            t.FromToSiteID = s.Type;
        ...
        private void UpdatingDetail(CA_Trans t, CA10100_pgDetail_Result s, bool isNew)
        {
            if (isNew)
            {             
            ...
            t.TaxID = s.TaxID;
            t.PayerReceiverAddr = s.PayerReceiverAddr;
            t.PayerReceiver =s.PayerReceiver;
            t.Transportation = s.Transportation;
