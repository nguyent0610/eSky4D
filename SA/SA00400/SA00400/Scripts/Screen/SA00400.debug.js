...
var menuClick = function (command) {
    switch (command) {
        ...
        case "new":
            if (HQ.isInsert) {
                HQ.grid.insert(App.grdSYS_ScreenCat);
            }
            break;
        case "delete":
...
var grdSYS_ScreenCat_ValidateEdit = function (item, e) {
    if (keys.indexOf(e.field) != -1) {
        if (HQ.grid.checkDuplicate(App.grdSYS_ScreenCat, e, keys)) {
...
var grdSYS_ScreenCat_Reject = function (record) {
    if (record.data.tstamp == '') {
        ...
        App.grdSYS_ScreenCat.getView().focusRow(App.stoData.getCount() - 1);
        App.grdSYS_ScreenCat.getSelectionModel().select(App.stoData.getCount() - 1);
    } else {
        record.reject();
    }
};
