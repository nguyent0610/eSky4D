...
function Check() {
    ...
    if (App.txtActiveCode.value == "") {
        HQ.message.show(1500, '', null);
        App.txtActiveCode.focus();
        return;
    }
    ...
    else if (App.txtActiveCode.value != HQ.activeCode){
        HQ.message.show(2018050801);
        return false;
    }
    save();
};
...
function save() {
    if (App.frmMain.isValid()) {
        App.frmMain.submit({
            waitMsg: 'Submiting...',
            url: 'SA03700/SA03700Save',
            params: {
                activeCode: HQ.activeCode
            },
            success: function (result, data) {
            ...
            , failure: function (errorMsg, data) {
                ...
                if (data.result.code) {
                    ...
                    HQ.message.process(errorMsg, data);
                }
            }
        });
    }
