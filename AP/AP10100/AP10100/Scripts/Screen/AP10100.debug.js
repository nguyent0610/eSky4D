...
var keys = ['TranAmt', 'UnitPrice', 'Qty'];
...
var stoChanged = function (sto) {
    if (!Ext.isEmpty(App.cboBatNbr.getValue()) || App.stoAP_Trans.data.length > 1) {
        App.cboVendID.setReadOnly(true);
        App.cboDocType.setReadOnly(true);
    }
    else {
...
var grdAP_Trans_ValidateEdit = function (item, e) {
    ...
    if (e.field == 'InvtId' && !Ext.isEmpty(e.value)) {
        if (HQ.grid.checkDuplicate(App.grdAP_Trans, e, ['InvtId'])) {
...
var renderRowNumber = function (value, meta, record) {
    return App.stoAP_Trans.data.indexOf(record) + 1;
}
var expand = function (cbo, delimiter) {
    var value = App.slmgrdAP_Trans.selected.items[0].data;
