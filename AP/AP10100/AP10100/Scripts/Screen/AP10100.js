    ...
    switch (command) {
        case "first":
            if (_focusrecord == 3) {
                ...
                App.SelectionRowOnGridTab1.select(0);
            }
        ...
        case "prev":
            if (_focusrecord == 3) {
                ...
                App.SelectionRowOnGridTab1.selectPrevious();
            }
        ...
        case "next":
            if (_focusrecord == 3) {
                ...
                App.SelectionRowOnGridTab1.selectNext();
            }
            break;
        case "last":
            if (_focusrecord == 3) {
                ...
                App.SelectionRowOnGridTab1.select(App.storeGrid.getCount() - 1);
            }
                ...
                if (_focusrecord == 2 )
                {
                    if (App.tabDocument.getForm().getRecord() != null) {
                        ...
                        if (App.storeGrid.getCount() == 0) {
                            ...
                            App.SelectionRowOnGridTab1.selected.items[0].set('TranDesc', vendIDAndDescr)
                            App.grd.editingPlugin.startEditByPosition({ row: App.storeGrid.getCount() - 1, column: 1 });

                        } else if (App.SelectionRowOnGridTab1.selected.items[0].data.LineType != "") {
                            App.storeGrid.insert(App.storeGrid.getCount(), Ext.data.Record());
                            App.SelectionRowOnGridTab1.select(App.storeGrid.getCount() - 1);
                            for (var i = 0; i < App.cboVendID.getStore().data.length; i++) {
                                if (App.cboVendID.getStore().data.items[i].data.VendID == App.cboVendID.value) {
            ...
            if (isDelete) {
                if (App.cboBatNbr.value != "") {
                    ...
                    if (_focusrecord == 3) {
                        ...
                        callMessage(11, '', 'deleteRecordFormBotAP_Doc')
                    }
                }
            }
            break;
...
function Save() {
    ...
    for (var i = 0; i <= App.storeGrid.getCount() - 1; i++) {
        App.SelectionRowOnGridTab1.select(i);
        //lay gia tri lineRef
        var lineRef = "";
        if (i < 10) {
        ...
        for (var k = 0; k <= App.storeGridTopTab2.getCount() - 1; k++) {
            App.SelectionRowOnGridTopTab2.select(k);
            if (App.SelectionRowOnGridTopTab2.selected.items[0].data.LineRef == App.storeGrid.data.items[i].data.LineRef) {
                //bien count de dem xem co bao nhieu cai cung LineRef de save vao cac bien TaxAmt 0 > 3 va bien txblAmt 0 > 3 cho dung 
                countTaxAmt++;
                if (countTaxAmt == 1) {
                    //taxAmt[0] = parseFloat(App.SelectionRowOnGridTopTab2.selected.items[0].data.TaxAmt);
                    //txblAmt = parseFloat(App.SelectionRowOnGridTopTab2.selected.items[0].data.TxblAmt);
                    App.storeGrid.data.items[i].set('TaxAmt00', parseFloat(App.SelectionRowOnGridTopTab2.selected.items[0].data.TaxAmt));
                    ...
                    App.storeGrid.data.items[i].set('TxblAmt00', parseFloat(App.SelectionRowOnGridTopTab2.selected.items[0].data.TxblAmt));
                } else if (countTaxAmt == 2) {
                    App.storeGrid.data.items[i].set('TaxAmt01', parseFloat(App.SelectionRowOnGridTopTab2.selected.items[0].data.TaxAmt));
                    App.storeGrid.data.items[i].set('TxblAmt01', parseFloat(App.SelectionRowOnGridTopTab2.selected.items[0].data.TxblAmt));
                } else if (countTaxAmt == 3) {
                    App.storeGrid.data.items[i].set('TaxAmt02', parseFloat(App.SelectionRowOnGridTopTab2.selected.items[0].data.TaxAmt));
                    App.storeGrid.data.items[i].set('TxblAmt02', parseFloat(App.SelectionRowOnGridTopTab2.selected.items[0].data.TxblAmt));
                } else if (countTaxAmt == 4) {
                    App.storeGrid.data.items[i].set('TaxAmt03', parseFloat(App.SelectionRowOnGridTopTab2.selected.items[0].data.TaxAmt));
                    App.storeGrid.data.items[i].set('TxblAmt03', parseFloat(App.SelectionRowOnGridTopTab2.selected.items[0].data.TxblAmt));
                }
            }
    ...
    if (strKeyGrid.indexOf(e.field) != -1) {
        if (e.value != '' && isAllValidKey(App.storeGrid.getChangedData().Created) && isAllValidKey(App.storeGrid.getChangedData().Updated))
            ...
            var record = Ext.create("App.AP_TransResultModel", {
                //CustId: "",
                TranDesc: App.SelectionRowOnGridTab1.selected.items[0].data.TranDesc
...
var grd_ValidateEdit = function (item, e) {
    ...
    if (strKeyGrid.indexOf(e.field) != -1)
    {
        if (duplicated(App.storeGrid, e))
        {
            ...
            App.SelectionRowOnGridTab1.selected.items[0].set('InvtID', '');
            return false;
        }
    }
    return true;
...
var loadDataAutoHeaderTop = function () {
    ...
    //else {
    //    App.dataFormTop.getForm().loadRecord(""); // cai nay can sua ////////////////////////////////////////////////////////////
    //    App.cboRefNbr.getStore().reload();
    //    setTimeout(function () { waitStoreRefNbrReLoad(); }, 1000);
    //}
};
...
var waitStoreGridTab1ReLoad = function () {
    ...
    App.tabAtAP10100.setActiveTab(App.tabDocument); // set tab dang su dung sang tab 1
    ...
    //App.SelectionRowOnGridTopTab2.select(0);
    App.tabAtAP10100.setActiveTab(App.tabDocument); // set tab dang su dung sang tab 1
    //App.SelectionRowOnGridTab1.select(0);//chon dong 1 cua grid 1 tab 1
...
var cboTaxCat_Change = function (item) {
    App.SelectionRowOnGridTab1.selected.items[0].set('TaxCat', item.value);
    var index = App.SelectionRowOnGridTab1.selected.items[0].index;
    ...
    if (index == undefined && App.SelectionRowOnGridTab1.selected.items[0].data.InvtID != "") {
        for (var i = 0; i < App.storeGrid.data.length; i++) {
            if (App.storeGrid.data.items[i].data.InvtID == App.SelectionRowOnGridTab1.selected.items[0].data.InvtID) {
                index = i;
            }
        }
    }
...
var cboTaxID_Change = function (item) {//obj = App.cboTaxID
    //obj.value
    App.SelectionRowOnGridTab1.selected.items[0].set('TaxId01', "");
    App.SelectionRowOnGridTab1.selected.items[0].set('TaxId02', "");
    App.SelectionRowOnGridTab1.selected.items[0].set('TaxId03', "");
    for (var i = 0; i < item.value.length; i++) {
        
        if (i == 0) {
            App.SelectionRowOnGridTab1.selected.items[0].set('TaxId00', item.value[0]);
        } else if (i == 1) {
            App.SelectionRowOnGridTab1.selected.items[0].set('TaxId01', item.value[1]);
        } else if (i == 2) {
            App.SelectionRowOnGridTab1.selected.items[0].set('TaxId02', item.value[2]);
        } else if (i == 3) {
            App.SelectionRowOnGridTab1.selected.items[0].set('TaxId03', item.value[3]);
        }
    }
    var index = App.SelectionRowOnGridTab1.selected.items[0].index;
    //xet dieu kien neu them moi thi index se bat khac di , do LineType khac rong se tao ra dong moi nen neu LineType chua co ta bat index tu dong la getCount()-1
...
var txtQty_Change = function (item) {
    ...
    var unitprice = App.SelectionRowOnGridTab1.selected.items[0].data.UnitPrice;
    var index = App.SelectionRowOnGridTab1.selected.items[0].index;
    //xet dieu kien neu them moi thi index se bat khac di , do LineType khac rong se tao ra dong moi nen neu LineType chua co ta bat index tu dong la getCount()-1
    if (index == undefined && App.SelectionRowOnGridTab1.selected.items[0].data.InvtID != "") {
        for (var i = 0; i < App.storeGrid.data.length; i++) {
    ...
    } else if (index == undefined && App.SelectionRowOnGridTab1.selected.items[0].data.InvtID == "") {
        index = 0;
    }
    App.SelectionRowOnGridTab1.selected.items[0].set('TranAmt', quantity * unitprice);
    //tai lai du lieu cua grid 1 va 2 cua tab 2 do moi cap nhap o grid 1 tab 1
    reloadDataGrid1AndGrid2Tab2(index);
    //reload lai tong tien , so tien no goc , so du chung tu (TotAmt, OrigDocAmt , DocBal)
    reloadAmountMustPayTotal(index);
...
var txtUnitPrice_Change = function (item) {
    //bat cac gia tri de chuan bi tinh toan lai
    var quantity = App.SelectionRowOnGridTab1.selected.items[0].data.Qty;
...
var txtTranAmt_Change = function (item) {
    ...
    App.SelectionRowOnGridTab1.selected.items[0].set('UnitPrice', tranAmt / quantity);
    //tai lai du lieu cua grid 1 va 2 cua tab 2 do moi cap nhap o grid 1 tab 1
    reloadDataGrid1AndGrid2Tab2(index);
    //reload lai tong tien , so tien no goc , so du chung tu (TotAmt, OrigDocAmt , DocBal)
    reloadAmountMustPayTotal(index);
...
var reloadDataGrid1AndGrid2Tab2 = function (index) {
    ...
    fillDataIntoGrid2Tab2();
    App.tabAtAP10100.setActiveTab(App.tabDocument); // set tab dang su dung sang tab 1
    App.SelectionRowOnGridTab1.select(index);
}
//reload lai tong tien , so tien no goc , so du chung tu (TotAmt, OrigDocAmt , DocBal)
var reloadAmountMustPayTotal = function(index){
    ...
    for (var i = 0; i <= App.storeGridBotTab2.getCount() - 1; i++) {
        App.SelectionRowOnGridBotTab2.select(i);
        totalTaxMustPay = totalTaxMustPay + parseFloat(App.SelectionRowOnGridBotTab2.selected.items[0].data.TaxAmt2);
    }
    //duyet grid 1 de lay so tien goc cua minh
    for (var k = 0; k <= App.storeGrid.getCount() - 1; k++) {
        App.SelectionRowOnGridTab1.select(k);
        totalAmountMustPay = totalAmountMustPay + App.SelectionRowOnGridTab1.selected.items[0].data.TranAmt;
    }
...
var fillDataIntoGrid1Tab2 = function () {
    ...
    for (var i = 0; i <= App.storeGrid.getCount() - 1; i++) {
        ...
        var tranAmt = App.SelectionRowOnGridTab1.selected.items[0].data.TranAmt;
        var taxCat = App.SelectionRowOnGridTab1.selected.items[0].data.TaxCat;
        var taxID0 = App.SelectionRowOnGridTab1.selected.items[0].data.TaxId00;
        var taxID1 = App.SelectionRowOnGridTab1.selected.items[0].data.TaxId01;
        var taxID2 = App.SelectionRowOnGridTab1.selected.items[0].data.TaxId02;
        var taxID3 = App.SelectionRowOnGridTab1.selected.items[0].data.TaxId03;
        //ham xoa cac dong co taxId01 , 02 va gop vao chung voi taxId00
        //App.SelectionRowOnGridTab1.select(i);
        //if (App.SelectionRowOnGridTab1.selected.items[0].data.TaxId01 != "" && App.SelectionRowOnGridTab1.selected.items[0].data.TaxId01 == "") {
        //    App.SelectionRowOnGridTab1.selected.items[0].set('TaxId00', (taxID0 + "," + taxID1).toString());
        //    App.SelectionRowOnGridTab1.selected.items[0].set('TaxId01', "");
        //} else if (App.SelectionRowOnGridTab1.selected.items[0].data.TaxId01 != "" && App.SelectionRowOnGridTab1.selected.items[0].data.TaxId01 != "") {
        //    App.SelectionRowOnGridTab1.selected.items[0].set('TaxId00', (taxID0 + "," + taxID1 + "," + taxID2).toString());
        //    App.SelectionRowOnGridTab1.selected.items[0].set('TaxId01', "");
        //    App.SelectionRowOnGridTab1.selected.items[0].set('TaxId02', "");
        //}
        //sau khi bat xong chuyen sang tab 2
        //App.tabAtAP10100.setActiveTab(App.tabDocument);
        //App.SelectionRowOnGridTab1.select();
        //App.SelectionRowOnGridTab1.selected.items[0].data.TaxId00;
...
var fillDataIntoGrid2Tab2 = function () {
    ...
    for (var i = 0; i <= App.storeGridTopTab2.getCount() - 1; i++) {
        App.SelectionRowOnGridTopTab2.select(i);
        ...
        if (App.SelectionRowOnGridTopTab2.selected.items[0].data.TaxID == "IVAT05") {
            iVat05 = "1";
            taxAmtTotalIVAT05 = taxAmtTotalIVAT05 + parseFloat(App.SelectionRowOnGridTopTab2.selected.items[0].data.TaxAmt)
            tranAmtTotalIVAT05 = tranAmtTotalIVAT05 + parseFloat(App.SelectionRowOnGridTopTab2.selected.items[0].data.TxblAmt)
        } else if (App.SelectionRowOnGridTopTab2.selected.items[0].data.TaxID == "IVAT10") {
            iVat10 = "1";
            taxAmtTotalIVAT10 = taxAmtTotalIVAT10 + parseFloat(App.SelectionRowOnGridTopTab2.selected.items[0].data.TaxAmt)
            tranAmtTotalIVAT10 = tranAmtTotalIVAT10 + parseFloat(App.SelectionRowOnGridTopTab2.selected.items[0].data.TxblAmt)
        } else if (App.SelectionRowOnGridTopTab2.selected.items[0].data.TaxID == "NONEVAT") {
            noneVat = "1";
            tranAmtTotalNoneVat = tranAmtTotalNoneVat + parseFloat(App.SelectionRowOnGridTopTab2.selected.items[0].data.TxblAmt)
        } else if (App.SelectionRowOnGridTopTab2.selected.items[0].data.TaxID == "OVAT10-00") {
            oVat10 = "1";
            taxAmtTotalOVAT10 = taxAmtTotalOVAT10 + parseFloat(App.SelectionRowOnGridTopTab2.selected.items[0].data.TaxAmt)
            tranAmtTotalOVAT10 = tranAmtTotalOVAT10 + parseFloat(App.SelectionRowOnGridTopTab2.selected.items[0].data.TxblAmt)
        } else if (App.SelectionRowOnGridTopTab2.selected.items[0].data.TaxID == "VAT10") {
            vat10 = "1";
            taxAmtTotalVAT10 = taxAmtTotalVAT10 + parseFloat(App.SelectionRowOnGridTopTab2.selected.items[0].data.TaxAmt)
            tranAmtTotalVAT10 = tranAmtTotalVAT10 + parseFloat(App.SelectionRowOnGridTopTab2.selected.items[0].data.TxblAmt)
        }
    }
    //khai bao cac record cho tung truong hop
...
//        App.SelectionRowOnGridTab1.selected.items[0].set('TaxId00', item.value[0]);
//        App.SelectionRowOnGridTab1.selected.items[0].set('TaxId01', "IVAT05");
//        App.SelectionRowOnGridTab1.selected.items[0].set('TaxId02', "IVAT10");
//    } else if (item.rawValue.search(", IVAT05, NONEVAT") != -1) {
//        App.SelectionRowOnGridTab1.selected.items[0].set('TaxId00', item.value[0]);
//        App.SelectionRowOnGridTab1.selected.items[0].set('TaxId01', "IVAT05");
//        App.SelectionRowOnGridTab1.selected.items[0].set('TaxId02', "NONEVAT");
//    } else if (item.rawValue.search(", IVAT10, IVAT05") != -1) {
//        App.SelectionRowOnGridTab1.selected.items[0].set('TaxId00', item.value[0]);
//        App.SelectionRowOnGridTab1.selected.items[0].set('TaxId01', "IVAT10");
//        App.SelectionRowOnGridTab1.selected.items[0].set('TaxId02', "IVAT05");
...
//        App.SelectionRowOnGridTab1.selected.items[0].set('TaxId00', item.value[0]);
//        App.SelectionRowOnGridTab1.selected.items[0].set('TaxId01', "NONEVAT");
//        App.SelectionRowOnGridTab1.selected.items[0].set('TaxId02', "IVAT05");
...
//    //reload lai tong tien , so tien no goc , so du chung tu (TotAmt, OrigDocAmt , DocBal)
//    reloadAmountMustPayTotal(index);
//    var taxid00 = App.SelectionRowOnGridTab1.selected.items[0].data.TaxId00;
//    //if(taxid00.search(","){
//    //    var taxid00change = taxid00.substring(0, r.data.TaxId00.search(","));
//    //    r.set('TaxId00', taxid00change);
//    //}
