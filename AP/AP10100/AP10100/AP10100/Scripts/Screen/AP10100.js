...
function Save() {
    ...
    if (App.frmMain.isValid()) {
        App.frmMain.submit({
            ...
            url: 'AP10100/Save',
            params: {
                lstheader_Batch: Ext.encode(App.stoAP10100_pdHeader.getRecordsValues()),
            ...
            success: function (msg, data) {
                ...
                if (BatNbr != '') {
                    App.cboBatNbr.getStore().load(function () {
                        ...
                        } else App.stoAP10100_pdHeader.reload();
...
var deleteRecordFormTopBatch = function (item) {
    if (item == 'yes') {
        App.frmMain.submit({
            ...
            url: 'AP10100/DeleteFormTopBatch',
            params: {
...
var firstLoad = function () {
    ...
    App.cboBatNbr.getStore().load(function () { App.stoAP10100_pdHeader.reload(); });

};
var frmChange = function () {
    if (App.stoAP10100_pdHeader.data.length > 0) {
        App.frmMain.getForm().updateRecord();
        HQ.isChange = HQ.store.isChange(App.stoAP10100_pdHeader) == false ? HQ.store.isChange(App.stoAR_Trans) : true;
...
var stoAP10100_pdHeader_Load = function (store) {
    ...
    if (store.data.length == 0) {
        ...
        //record.data.EditScrnNbr = 'AP10100';
...
var cboBatNbr_Select = function (sender, value) {
    if (sender.valueModels != null && !App.stoAP10100_pdHeader.loading) {
...
var loadstoAP10100_pgLoadTaxTrans = function () {
...
function delTaxMutil() {
    ...
    for (var i = App.stoAP10100_pgLoadTaxTrans.data.length - 1; i >= 0; i--) {
        var data = HQ.store.findInStore(App.stoAR_Trans, ['LineRef'], [App.stoAP10100_pgLoadTaxTrans.data.items[i].data.LineRef])
