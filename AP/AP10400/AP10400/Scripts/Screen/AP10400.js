                ...
                if (_focusrecord == 2 )
                {
                    ...
                    if (App.cboBatNbr.value != "" && App.cboRefNbr!= "") {
                        ...
                        if(App.storeGrid.getCount() == 0) {
                            ...
                            App.SelectionRowOnGrid.select(App.storeGrid.getCount() - 1);
                            App.SelectionRowOnGrid.selected.items[0].set('DocDate', time);

                            App.grd.editingPlugin.startEditByPosition({ row: App.storeGrid.getCount() - 1, column: 1 });
                        } else if (App.SelectionRowOnGrid.selected.items[0].data.InvcNbr != "") {
                            App.storeGrid.insert(App.storeGrid.getCount(), Ext.data.Record());
                            App.SelectionRowOnGrid.select(App.storeGrid.getCount() - 1);
                            App.SelectionRowOnGrid.selected.items[0].set('DocDate', time);
...
var waitcboRefNbrForInsert = function () {
    ...
    App.dataFormBotRight.getForm().loadRecord(App.storeFormBotRight.getAt(0));
...
function Save() {
    ...
    App.dataFormBotRight.getForm().updateRecord();
    ...
    if (App.dataForm.isValid()) {
        App.dataForm.submit({
            ...
            params: {
                ...
                lstheaderBotRight: Ext.encode(App.storeFormBotRight.getChangedData({ skipIdForPhantomRecords: false })),
        ...
        try {
            App.direct.DeleteFormTopBatch(App.cboBatNbr.getValue(), App.txtBranchID.getValue(), {
                success: function (data) {
                    ...
                    App.cboRefNbr.getStore().reload();
                    App.cboRefNbr.setValue('');
                    App.dataFormBotRight.getForm().reset(true);
...
var grd_ValidateEdit = function (item, e) {
    ...
    if (strKeyGrid.indexOf(e.field) != -1)
    {
        if (duplicated(App.storeGrid, e))
        {
            ...
            App.SelectionRowOnGrid.selected.items[0].set('InvcNbr', '');
            return false;
        }
    }
    return true;
...
var loadDataAutoHeaderBotRight = function () {
    ...
    if (record) {
        App.dataFormBotRight.getForm().loadRecord(record);
        //App.cboRefNbr.getStore().reload();
        //setTimeout(function () { waitStoreRefNbrReLoad(); }, 1000);
    }
};
...
var txtPaymentGrid_change = function (item, newValue, oldValue) {
    for (var i = 0; i < App.storeGrid.data.length; i++) {
        ...
        if (App.SelectionRowOnGrid.selected.items[0].data == App.storeGrid.data.items[i].data) {
...
var cboInvcNbr_change = function (item,newValue,oldValue) {
    for(var i = 0;i < App.cboInvcNbr.store.data.length; i++) {
        if (App.cboInvcNbr.getStore().data.items[i].data.InvcNbr == newValue) {
            App.SelectionRowOnGrid.selected.items[0].set('DocBal', App.cboInvcNbr.getStore().data.items[i].data.DocBal);
            App.SelectionRowOnGrid.selected.items[0].set('OrigDocBal', App.cboInvcNbr.getStore().data.items[i].data.OrigDocBal);
            App.SelectionRowOnGrid.selected.items[0].set('VendID', App.cboInvcNbr.getStore().data.items[i].data.VendID);
            App.SelectionRowOnGrid.selected.items[0].set('Descr', App.cboInvcNbr.getStore().data.items[i].data.Descr);
            App.SelectionRowOnGrid.selected.items[0].set('BatNbr', App.cboInvcNbr.getStore().data.items[i].data.BatNbr);
            App.SelectionRowOnGrid.selected.items[0].set('RefNbr', App.cboInvcNbr.getStore().data.items[i].data.RefNbr);
        }
    }
