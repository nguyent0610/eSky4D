...
namespace AP10400.Controllers
{
    ...
    public class AP10400Controller : Controller
    {
        ...
        public ActionResult Save(FormCollection data, string branchID, string handle, string batNbr, string refNbr,string reasonCD)
        {
            ...
            foreach (AP_Doc created in lstheaderBotRight.Created)
            {
                ...
                if (objHeader == null)
                {
                    ...
                    if (tmpBatNbr != "")
                    {
                        ...
                        if (objHeaderBatNbr == null)
                        {
                            ...
                            objHeaderBatNbr.EditScrnNbr = "AP10400";
            ...
            foreach (AP_Doc updated in lstheaderBotRight.Updated)
            {
                // Get the image path
        ...
        public ActionResult DeleteFormTopBatch(string batNbr, string branchID)
        {
            ...
            if (recordTopBatch != null)
            {
                ...
                if (recordBotAP_Doc != null)
                {
                    ...
                    var recordGridAP_Adjust = _db.AP_Adjust.Where(p => p.BranchID == branchID && p.BatNbr == batNbr).ToList();
                    if (recordGridAP_Adjust != null)
                    {
                        for (int l = 0; l < recordGridAP_Adjust.Count; l++)
                        {
                            _db.AP_Adjust.DeleteObject(recordGridAP_Adjust[l]);
