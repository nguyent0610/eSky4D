                        ...
                        stkeyFilter.split(',').forEach(function (key) {
                            if (key) {
                                if ((typeof passNull(value)) == "string") {
                                    var fieldData = record.data[key].toString().toLowerCase().indexOf(passNull(value).toLowerCase());
                                    if (fieldData > -1) {
                                        isMap = true;
                                        return record;
                                    }
...
//TrungHT 
function MoveFirst(combo,key) {
    combo.setValue(combo.getStore().first().get(key));
};
function MoveLast(combo, key) {
    combo.setValue(combo.getStore().last().get(key));
};
function MoveNext(combo, key) {
    
    var index = combo.getStore().findExact(key, combo.getValue());
    if(combo.getStore().data.length>index + 1)
    combo.setValue(combo.getStore().getAt(index + 1).get(key));
};
function MovePrev(combo, key) {
