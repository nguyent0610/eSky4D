    ...
    switch (command) {
        ...
        case "new":
            if (isInsert) {
                ...
                if (_focusrecord == 2 )
                {
                    if (App.dataFormBot.getForm().getRecord() != null) {
                        App.SelectionRowOnGrid.select(App.storeGrid.getCount() - 1);
                        ...
                        }else if(App.SelectionRowOnGrid.selected.items[0].data.TranAmt != ""){
                            App.storeGrid.insert(App.storeGrid.getCount(), Ext.data.Record());
                            App.SelectionRowOnGrid.select(App.storeGrid.getCount() - 1);
                            for (var i = 0; i < App.cboCustId.getStore().data.length; i++) {
                                if (App.cboCustId.getStore().data.items[i].data.CustID == App.cboCustId.value) {
...
var waitcboRefNbrForInsert = function () {
    ...
    App.dataFormBot.getForm().loadRecord(App.storeFormBot.getAt(0));
...
function Save() {
    ...
    App.dataFormBot.getForm().updateRecord();
        ...
        try {
            App.direct.DeleteFormTopBatch(App.cboBatNbr.getValue(), App.txtBranchID.getValue(), {
                success: function (data) {
                    ...
                    App.cboRefNbr.getStore().reload();
                    App.cboRefNbr.setValue('');
                    App.dataFormBot.getForm().reset(true);
