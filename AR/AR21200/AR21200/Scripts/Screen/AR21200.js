var menuClick = function (command) {
    switch (command) {
        case "first":       
            App.SelectionModelAR_Location.select(0);
            break;
        case "prev":
            App.SelectionModelAR_Location.selectPrevious();
            break;
        case "next":
            App.SelectionModelAR_Location.selectNext();
            break;
        case "last":
            App.SelectionModelAR_Location.select(App.Store1.getCount() - 1);
            break;
        case "refresh":
...
var isAllValid=function(items){
    if(items!=undefined){
        for(var i=0; i<items.length; i++){
            if(items[i]["Location"]==undefined) continue;
            if(items[i]["Location"].trim()=="")
            {
                callMessage(15,'@Util.GetLang("Location")',null);
                return false;
            }
