...
var waitcboRefNbrForInsert = function () {
    ...
    var record = Ext.create("App.AR_DocClassModel", {
        //CustId: "",
...
var grd_Edit = function (item, e) {
    ...
    if (keys.indexOf(e.field) != -1) {
        if (e.value != '' && isAllValidKey(App.storeGrid.getChangedData().Created) && isAllValidKey(App.storeGrid.getChangedData().Updated)) {
            ...
            var time = new Date();
            App.slmGrid.selected.items[0].set('DocDate', time);
            var record = Ext.create("App.AR10200_BindingGrid_ResultModel", {
                //CustId: "",
                DocDate: time,
            });
...
var chkAllowSort_change = function (obj) {
    App.grd.sortableColumns = obj.checked;
    App.grd.update();
};
...
var cboInvcNbr_change = function (item, newValue, oldValue) {
    for (var i = 0; i < App.cboInvcNbr.store.data.length; i++) {
        if (App.cboInvcNbr.getStore().data.items[i].data.InvcNbr == newValue) {
            ...
            App.slmGrid.selected.items[0].set('CustId', App.cboInvcNbr.getStore().data.items[i].data.CustId);
            App.slmGrid.selected.items[0].set('CustName', App.cboInvcNbr.getStore().data.items[i].data.CustName);
            App.slmGrid.selected.items[0].set('SlsperID', App.cboInvcNbr.getStore().data.items[i].data.SlsperID);
