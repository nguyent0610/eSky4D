    ...
    switch (command) {
        case "first":
            if (_focusrecord == 3) {
                ...
                App.SelectionRowOnGrid1.select(0);
            } else if (_focusrecord == 2) {
                App.SelectionRowOnGrid2.select(0);
            }
        ...
        case "prev":
            if (_focusrecord == 3) {
                ...
                App.SelectionRowOnGrid1.selectPrevious();
            } else if (_focusrecord == 2) {
                App.SelectionRowOnGrid2.selectPrevious();
            }
        ...
        case "next":
            if (_focusrecord == 3) {
                ...
                App.SelectionRowOnGrid1.selectNext();
            } else if (_focusrecord == 2) {
                App.SelectionRowOnGrid2.selectNext();
            }
            break;
        case "last":
            if (_focusrecord == 3) {
                ...
                App.SelectionRowOnGrid1.select(App.storeGrid1.getCount() - 1);
            } else if (_focusrecord == 2) {
                App.SelectionRowOnGrid2.select(App.storeGrid2.getCount() - 1);
            }
    ...
    if (App.dataForm.isValid()) {
        App.dataForm.submit({
            ...
            params: {
                ...
                lstgrd1: Ext.encode(App.storeGrid1.getChangedData({ skipIdForPhantomRecords: false })),
                lstgrd2: Ext.encode(App.storeGrid2.getChangedData({ skipIdForPhantomRecords: false })),
            ...
            success: function (result, data) {
                ...
                if (data.result.value1 == "") {
                    ...
                    setTimeout(function () { AfterSaveWaitBatNbrStoreReload(data.result.value1.toString()); }, 1500);
...
var cboCustId_Change = function () {
    if (App.cboCustId.value != null && App.cboDocType.value != null) {
...
var txtPaymentGrid1_change = function (item, newValue, oldValue) {
    ...
    for (var i = 0; i < App.storeGrid1.data.length; i++) {
        //tmpApplicationTotal += App.storeGrid2.data.items[i].data.Payment;
        if (App.SelectionRowOnGrid1.selected.items[0].data == App.storeGrid1.data.items[i].data) {
...
var txtPaymentGrid2_change = function (item, newValue, oldValue) {
    for (var i = 0; i < App.storeGrid2.data.length; i++) {
        ...
        if (App.SelectionRowOnGrid2.selected.items[0].data == App.storeGrid2.data.items[i].data)
        {
                //dk neu gia tri moi edit cua payment lon hon gia tri original cua hang do thi set payment = gia tri original , set docbal = 0 va check vao checkbox hang do 
                if (newValue >= App.storeGrid2.data.items[i].data.OrigDocBal) 
                {
                    //if (App.storeGrid2.data.items[i].data.DocBal != 0) {
