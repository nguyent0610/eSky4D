...
namespace INProcess
{
    public class IN 
    {
                ...
                foreach (DataRow tran in lstTrans.Rows)
                {
                    ...
                    if (objInvt.StkItem == 1 && objInvt.LotSerTrack.PassNull() != string.Empty && objInvt.LotSerTrack.PassNull() != "N")
                    {
                        ...
                        foreach (DataRow lotRow in dtLot.Rows)
                        {
                            ...
                            {
                                objItemLot.QtyShipNotInv = Math.Round(objItemLot.QtyShipNotInv + (lotRow.String("TranType") == "IN" ? qty : 0), 0);
                                objItemLot.QtyOnHand = Math.Round(objItemLot.QtyOnHand + qty, 0);
                ...
                foreach (DataRow tran in lstTrans.Rows)
                {
                    ...
                    if (objInvt.StkItem == 1 && objInvt.LotSerTrack.PassNull() != string.Empty && objInvt.LotSerTrack.PassNull() != "N")
                    {
                        ...
                        foreach (DataRow lotRow in dtLot.Rows)
                        {
                            ...
                            {
                                if (objItemLot.MfgrLotSerNbr.Trim() != lotRow.String("MfgrLotSerNbr").Trim())
                                {
                                    throw new MessageException(MessageType.Message, "2018042350", "", new[] { lotRow.String("MfgrLotSerNbr"), objSite.SiteID + "-" + objItemLot.LotSerNbr });
                                }
                            }
        ...
        private void Insert_IN_ItemLot(ref clsIN_ItemLot objIN_ItemLot, string InvtID, string SiteID, string LotSerNbr, DateTime ExpDate, string MfgrLotSerNbr, double Cost, double Qty)
        {
            try
            {
        ...
        public bool IN11200_Release(string branchID, string batNbr, string transType, string toSiteID)
        {
            try
            {
                ...
                {
                    objInvt.GetByKey(tran.String("InvtID"));
                    objSite.GetByKey(tran.String("InvtID"), tran.String("BranchID"));
                ...
                {
                    objInvt.GetByKey(tran.String("InvtID"));
                    if (!objSite.GetByKey(tran.String("InvtID"), toSiteID)) //tran.String("SiteID")
                    {
                        throw new MessageException(MessageType.Message, "606");
                    }
                    ...
                    if (objBranchSite != null)
                    {
                        double tranQty = -qty;//Math.Abs(qty);
                        if (!objBranchSite.GetByKey(tran.String("InvtID"), toSiteID))
                        {
                            Insert_IN_ItemSite(ref objBranchSite, tran.String("InvtID"), objInvt.StkItem, toSiteID, tranQty);
                        }
                        ...
                        {
                            objBranchSite.QtyOnHand = Math.Round(objBranchSite.QtyOnHand + tranQty, 0);
                            objBranchSite.QtyAllocIN = Math.Round(objBranchSite.QtyAllocIN + tranQty, 0);
                            objBranchSite.LUpd_DateTime = DateTime.Now;
                            objBranchSite.LUpd_Prog = Prog;
                            objBranchSite.LUpd_User = User;
                            objBranchSite.Update();
                    ...
                    if (objInvt.StkItem == 1 && objInvt.LotSerTrack.PassNull() != string.Empty && objInvt.LotSerTrack.PassNull() != "N")
                    {
                        ...
                        {
                            qty = Math.Round(lotRow.Double("Qty") * (lotRow.String("UnitMultDiv") == "D" ? 1.0 / lotRow.Double("CnvFact") : lotRow.Double("CnvFact")), 0) * lotRow.Short("InvtMult");
                            if (!objItemLot.GetByKey(toSiteID, lotRow.String("InvtID"), lotRow.String("LotSerNbr"))) //lotRow.String("BranchID")
                            {
                                objItemLot.Reset();
                                objItemLot.LotSerNbr = lotRow.String("LotSerNbr");
                                objItemLot.InvtID = lotRow.String("InvtID");
                                objItemLot.SiteID = toSiteID;//lotRow.String("BranchID");
                                objItemLot.ExpDate = lotRow.Date("ExpDate");
                                objItemLot.WarrantyDate = lotRow.Date("WarrantyDate");
                                objItemLot.LIFODate = lotRow.Date("ExpDate");
                            ...
                            if (objBranchLot != null)
                            {
                                double transLotQty = -qty;//Math.Abs(qty);
                                if (!objBranchLot.GetByKey(toSiteID, tran.String("InvtID"), lotRow.String("LotSerNbr")))
                                {
                                    Insert_IN_ItemLot(ref objBranchLot, tran.String("InvtID"), toSiteID, objItemLot.LotSerNbr, objItemLot.ExpDate, objItemLot.MfgrLotSerNbr, 0.0, transLotQty);
                                }
                                ...
                                {
                                    objBranchLot.QtyOnHand = Math.Round(objBranchLot.QtyOnHand + transLotQty, 0);
                                    objBranchLot.QtyAllocIN = Math.Round(objBranchLot.QtyAllocIN + transLotQty, 0);
                                    objBranchLot.LUpd_DateTime = DateTime.Now;
                                    objBranchLot.LUpd_Prog = Prog;
                                    objBranchLot.LUpd_User = User;
                                    objBranchLot.Update();
                ...
                sql.IN11500_ReleaseBatch(branchID, batNbr, Prog, User, transType);
                return true;
            }
            catch (Exception ex)
            {
        ...
        public bool IN11200_Cancel(string branchID, string batNbr, string transType, string toSiteID, string handle, string fromSite)
        {
            try
            {
                string rcptNbr = string.Empty;
                ...
                foreach (DataRow tran in lstTrans.Rows)
                {
                    ...
                    if (!objItem.GetByKey(tran.String("InvtID"), toSiteID))
                    {
                        throw new MessageException(MessageType.Message, "606");
                    }
                    ...
                    if (objInvt.StkItem == 1)
                    {
                        ...
                        if (transType == "SC")
                        {
                            objItem.QtyAvail = Math.Round(objItem.QtyAvail + tranQty, 0);
                            objItem.QtyOnHand = Math.Round(objItem.QtyOnHand + tranQty, 0);
                    
                        }
                        else if (transType == "BC")
                        {
                            objItem.QtyAllocIN = Math.Round(objItem.QtyAllocIN + qty, 0);
                            objItem.QtyOnHand = Math.Round(objItem.QtyOnHand + qty, 0);
                        }
                    ...
                    {
                        double tranQty = Math.Abs(qty);
                        if (!objBranchSite.GetByKey(tran.String("InvtID"), fromSite) && (transType != "SC" || transType != "SB"))
                        {
                            throw new MessageException(MessageType.Message, "606"); //Insert_IN_ItemSite(ref objBranchSite, tran.String("InvtID"), objInvt.StkItem, toSiteID, tranQty);
                        }
                        //else if (objBranchSite.QtyAvail == 0 || objBranchSite.QtyOnHand == 0)
                        //{
                        //    throw new MessageException(MessageType.Message, "608", "", new[] { tran.String("InvtID"), fromSite });
                        //}
                        else
                        {
                            objBranchSite.QtyOnHand = Math.Round(objBranchSite.QtyOnHand + tranQty, 0);
                    ...
                    if (objInvt.StkItem == 1 && objInvt.LotSerTrack.PassNull() != string.Empty && objInvt.LotSerTrack.PassNull() != "N")
                    {
                        ...
                        foreach (DataRow lotRow in dtLot.Rows)
                        {
                            ...
                            if (transType == "SC")
                            {
                                objItemLot.QtyOnHand = Math.Round(objItemLot.QtyOnHand + tranQty, 0);
                                objItemLot.QtyAvail = Math.Round(objItemLot.QtyAvail + tranQty, 0);
                            ...
                            {
                                double transLotQty = Math.Abs(qty);
                                if (!objBranchLot.GetByKey(fromSite, tran.String("InvtID"), lotRow.String("LotSerNbr")) && transType == "BC")
                                {
                                    Insert_IN_ItemLot(ref objBranchLot, tran.String("InvtID"), fromSite, objItemLot.LotSerNbr, objItemLot.ExpDate, objItemLot.MfgrLotSerNbr, 0.0, 0.0);
                                    
                                }
                                if (!objBranchLot.GetByKey(fromSite, tran.String("InvtID"), lotRow.String("LotSerNbr")) && transType != "BC") //|| objBranchLot.QtyAvail == 0 || objBranchLot.QtyOnHand == 0
                                {
                                    throw new MessageException(MessageType.Message, "201508181", "", new[] { "", tran.String("InvtID") + " " + Util.GetLang("Site") + " " + fromSite, lotRow.String("LotSerNbr"), transLotQty.ToString() });
                                    //throw new MessageException(MessageType.Message, "606"); 
