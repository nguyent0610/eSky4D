    ...
    public class OM 
    {
		...
		public bool OM20500_ReleaseKAO(string branchID, string orderNbr, Dictionary<string, double> dicRef, string deliveryID, DateTime shipDate, DateTime docDate, bool isAddStock, Dictionary<string, double> dicRefLot)
		{
			...
			try
			{
                ...
                short ordNo = objSql.OM_GetOrderNoKAO(objPDAOrd.BranchID, objPDAOrd.SlsPerID, objPDAOrd.OrderDate);
        ...
        public bool OM20500_Release(string branchID, string orderNbr, Dictionary<string, double> dicRef, string deliveryID, DateTime shipDate, DateTime docDate, bool isAddStock, string remark, out string OMOrderNbr)
        {
            ...
            try
            {
                ...
                short ordNo = objSql.OM_GetOrderNo(objPDAOrd.BranchID, objPDAOrd.SlsPerID, objPDAOrd.OrderDate, shipDate);
                objSalesOrd.OrderNo = (short)(ordNo != 0 ? ordNo + 1 : 0);
                objSalesOrd.OrderType = objPDAOrd.OrderType;
                objSalesOrd.OrderWeight = objPDAOrd.OrderWeight;
                objSalesOrd.OrigOrderNbr = objPDAOrd.OrderNbr;
                ...
                clsOM_PDASalesOrdDet objPDADet = new clsOM_PDASalesOrdDet(Dal);
                clsOM_SalesOrdDet objSalesDet = new clsOM_SalesOrdDet(Dal);
                clsOM_PDASalesOrdDisc objOM_PDASalesOrdDisc = new clsOM_PDASalesOrdDisc(Dal);
                clsOM_SalesOrdGift objOM_SalesOrdGift = new clsOM_SalesOrdGift(Dal);
                foreach (var item in dicRef)
                {
                    //them cap nhat qua tang table OM_PDAOrdDisc 20150701
                    if (item.Key.Split(',').Length > 1)
                    {
                        if (objOM_PDASalesOrdDisc.GetByKey(orderNbr, item.Key.Split(',')[0], "02", "", branchID))
                        {
                            objOM_PDASalesOrdDisc.QtyHist = objOM_PDASalesOrdDisc.QtyHist == 0 ? objOM_PDASalesOrdDisc.Qty : objOM_PDASalesOrdDisc.QtyHist;
                            objOM_PDASalesOrdDisc.Qty = item.Value;
                            objOM_PDASalesOrdDisc.LUpd_DateTime = DateTime.Now;
                            objOM_PDASalesOrdDisc.LUpd_Prog = Prog;
                            objOM_PDASalesOrdDisc.LUpd_User = User;
                            objOM_PDASalesOrdDisc.Update();
                        }
                        else
                        {
                            objOM_PDASalesOrdDisc = new clsOM_PDASalesOrdDisc(Dal);
                            objOM_PDASalesOrdDisc.Reset();
                            objOM_PDASalesOrdDisc.Type = "02";
                            objOM_PDASalesOrdDisc.DiscType = item.Key.Split(',')[1];
                            objOM_PDASalesOrdDisc.OrderNbr = orderNbr;
                            objOM_PDASalesOrdDisc.BranchID = branchID;
                            objOM_PDASalesOrdDisc.GiftName = item.Key.Split(',')[2];
                            objOM_PDASalesOrdDisc.Code = item.Key.Split(',')[0];
                            objOM_PDASalesOrdDisc.Qty = item.Value;

                            objOM_PDASalesOrdDisc.Crtd_DateTime = DateTime.Now;
                            objOM_PDASalesOrdDisc.Crtd_Prog = Prog;
                            objOM_PDASalesOrdDisc.Crtd_User = User;
                            objOM_PDASalesOrdDisc.LUpd_DateTime = DateTime.Now;
                            objOM_PDASalesOrdDisc.LUpd_Prog = Prog;
                            objOM_PDASalesOrdDisc.LUpd_User = User;
                            objOM_PDASalesOrdDisc.Add();

                        }

                        if (objOM_SalesOrdGift.GetByKey(objSalesOrd.BranchID, objSalesOrd.OrderNbr, item.Key.Split(',')[0], "02", ""))
                        {
                            //objOM_SalesOrdGift.QtyHist = objOM_PDASalesOrdDisc.QtyHist == 0 ? objOM_PDASalesOrdDisc.Qty : objOM_PDASalesOrdDisc.QtyHist;
                            objOM_SalesOrdGift.Qty = item.Value;
                            objOM_SalesOrdGift.LUpd_DateTime = DateTime.Now;
                            objOM_SalesOrdGift.LUpd_Prog = Prog;
                            objOM_SalesOrdGift.LUpd_User = User;
                            objOM_SalesOrdGift.Update();
                        }
                        else
                        {
                            objOM_SalesOrdGift = new clsOM_SalesOrdGift(Dal);
                            objOM_SalesOrdGift.Reset();
                            objOM_SalesOrdGift.Type = "02";
                            objOM_SalesOrdGift.DiscType = item.Key.Split(',')[1];
                            objOM_SalesOrdGift.OrderNbr = objSalesOrd.OrderNbr;
                            objOM_SalesOrdGift.BranchID = objSalesOrd.BranchID;
                            objOM_SalesOrdGift.GiftName = item.Key.Split(',')[2];
                            objOM_SalesOrdGift.Code = item.Key.Split(',')[0];
                            objOM_SalesOrdGift.Qty = item.Value;

                            objOM_SalesOrdGift.Crtd_DateTime = DateTime.Now;
                            objOM_SalesOrdGift.Crtd_Prog = Prog;
                            objOM_SalesOrdGift.Crtd_User = User;
                            objOM_SalesOrdGift.LUpd_DateTime = DateTime.Now;
                            objOM_SalesOrdGift.LUpd_Prog = Prog;
                            objOM_SalesOrdGift.LUpd_User = User;
                            objOM_SalesOrdGift.Add();

                        }

                    }
                    //end 20150701
                    else if (objPDADet.GetByKey(branchID, orderNbr, item.Key))
                    {
                        double rate = item.Value / objPDADet.LineQty;
                        ...
                        objSalesDet.UnitRate = objPDADet.UnitRate;
                        objSalesDet.UnitWeight = objPDADet.UnitWeight;
                        //objSalesDet.Cumulative = objPDADet.Cumulative;
                        ...
                        if (objInvt.StkItem == 1 || objPDADet.BOType != "B")
                        {
                            ...
                            qtyTot = editQty;//20160129da cap nhat lai kho khi tru tung thang//+ CalculateInvtTotals(objSalesOrd, lstDet, objSalesDet.InvtID, objSalesDet.SiteID, objSalesDet.LineRef);
                ...
                //                  objSalesOrd.VolDiscAmt)) / 100, 0);
                //}
                curyOrdDiscAmt = objPDAOrd.OrdDiscAmt;
                objSalesOrd.OrdDiscAmt = curyOrdDiscAmt;
                objSalesOrd.OrdAmt = Math.Round(
                        txblAmt + objPDAOrd.FreightAllocAmt +
                        objPDAOrd.MiscAmt + taxAmt +
                        soFee - objSalesOrd.VolDiscAmt -
                        curyOrdDiscAmt, 0);
                if (objSalesOrd.OrdAmt < 0)
                    throw new MessageException(MessageType.Message, "2016030801", "", new[] { objSalesOrd.BranchID, objSalesOrd.OrderNbr });
                //objPDAOrd.Status = "O";
                objPDAOrd.Update();

                objSalesOrd.Add();

                //OM10100_PrintInvoice(objSalesOrd.BranchID, objSalesOrd.OrderNbr);//gọi tới OM40200
                //OM10100_InvoiceRelease(objSalesOrd.BranchID, objSalesOrd.OrderNbr, "R", DateTime.Now.ToDateShort());//gọi tới OM40100
                return true;
            }
            catch (Exception ex)
            {
