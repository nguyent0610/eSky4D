'-- ------------------------------------------------------------
'-- Class name    :  clsSMS_SalesOrdWrk
'-- Created date  :  10/2/2012
'-- Created by    :  
'-- Updated by    :  
'-- Generated by    :  Class Maker v1.0
'-- ------------------------------------------------------------

'-- Import Libraries --
Imports HQFramework.Common
Imports HQFramework.DAL
Public Class clsSMS_SalesOrdWrk
#Region "Constants"
	Private Const PP_SMS_SalesOrdWrk As String = "PP_SMS_SalesOrdWrk"
#End Region 

#Region "Member Variables"
	Private mvarBranchID As System.String

	Private mvarOrderNbr As System.String

	Private mvarOrderDate As System.DateTime

	Private mvarCustID As System.String

	Private mvarOrdAmt As System.Double

	Private mvarOrdStatus As System.Byte

	Private mvarSlsperId As System.String

	Private mvarPicFileName As System.String

	Private mvarPicFileCrtdDateTime As System.DateTime

	Private mvarCrtd_DateTime As System.DateTime

	Private mvarLUpd_DateTime As System.DateTime

#End Region 

	Private m_Dal As DataAccess
#Region "Constructors"
	Public Sub New()
		m_Dal = New DataAccess
		Reset()
	End Sub
	Public Sub New(ByVal dal As DataAccess)
		m_Dal = dal
		Reset()
	End Sub
#End Region 

#Region "Public Properties"
	Public Property BranchID() As System.String
		Get
			Return mvarBranchID
		End Get
		Set(ByVal Value As System.String)
			mvarBranchID = Value
		End Set
	End Property

	Public Property OrderNbr() As System.String
		Get
			Return mvarOrderNbr
		End Get
		Set(ByVal Value As System.String)
			mvarOrderNbr = Value
		End Set
	End Property

	Public Property OrderDate() As System.DateTime
		Get
			Return mvarOrderDate
		End Get
		Set(ByVal Value As System.DateTime)
			mvarOrderDate = Value
		End Set
	End Property

	Public Property CustID() As System.String
		Get
			Return mvarCustID
		End Get
		Set(ByVal Value As System.String)
			mvarCustID = Value
		End Set
	End Property

	Public Property OrdAmt() As System.Double
		Get
			Return mvarOrdAmt
		End Get
		Set(ByVal Value As System.Double)
			mvarOrdAmt = Value
		End Set
	End Property

	Public Property OrdStatus() As System.Byte
		Get
			Return mvarOrdStatus
		End Get
		Set(ByVal Value As System.Byte)
			mvarOrdStatus = Value
		End Set
	End Property

	Public Property SlsperId() As System.String
		Get
			Return mvarSlsperId
		End Get
		Set(ByVal Value As System.String)
			mvarSlsperId = Value
		End Set
	End Property

	Public Property PicFileName() As System.String
		Get
			Return mvarPicFileName
		End Get
		Set(ByVal Value As System.String)
			mvarPicFileName = Value
		End Set
	End Property

	Public Property PicFileCrtdDateTime() As System.DateTime
		Get
			Return mvarPicFileCrtdDateTime
		End Get
		Set(ByVal Value As System.DateTime)
			mvarPicFileCrtdDateTime = Value
		End Set
	End Property

	Public Property Crtd_DateTime() As System.DateTime
		Get
			Return mvarCrtd_DateTime
		End Get
		Set(ByVal Value As System.DateTime)
			mvarCrtd_DateTime = Value
		End Set
	End Property

	Public Property LUpd_DateTime() As System.DateTime
		Get
			Return mvarLUpd_DateTime
		End Get
		Set(ByVal Value As System.DateTime)
			mvarLUpd_DateTime = Value
		End Set
	End Property

#End Region 

#Region "Public Methods"
	Public Function Add() as Boolean 
		Dim DAL As DataAccess = m_Dal
		Try
			Dim pc As New ParamCollection
			pc.Add(New ParamStruct("@Action", DbType.String, "AddNew", ParameterDirection.Input,50 ))
			pc.Add(New ParamStruct("@BranchID",DbType.String, clsCommon.GetValueDBNull(BranchID), ParameterDirection.Input,30 ))
			pc.Add(New ParamStruct("@OrderNbr", DbType.String,clsCommon.GetValueDBNull(Me.mvarOrderNbr), ParameterDirection.Input,15 ))
			pc.Add(New ParamStruct("@OrderDate", DbType.DateTime,clsCommon.GetValueDBNull(Me.mvarOrderDate.Date), ParameterDirection.Input,16 ))
			pc.Add(New ParamStruct("@CustID", DbType.String,clsCommon.GetValueDBNull(Me.mvarCustID), ParameterDirection.Input,50 ))
			pc.Add(New ParamStruct("@OrdAmt", DbType.Decimal,clsCommon.GetValueDBNull(Me.mvarOrdAmt), ParameterDirection.Input,8 ))
			pc.Add(New ParamStruct("@OrdStatus", DbType.Byte,clsCommon.GetValueDBNull(Me.mvarOrdStatus), ParameterDirection.Input,1 ))
			pc.Add(New ParamStruct("@SlsperId", DbType.String,clsCommon.GetValueDBNull(Me.mvarSlsperId), ParameterDirection.Input,15 ))
			pc.Add(New ParamStruct("@PicFileName", DbType.String,clsCommon.GetValueDBNull(Me.mvarPicFileName), ParameterDirection.Input,100 ))
			pc.Add(New ParamStruct("@PicFileCrtdDateTime", DbType.DateTime,clsCommon.GetValueDBNull(Me.mvarPicFileCrtdDateTime), ParameterDirection.Input,16 ))
			pc.Add(New ParamStruct("@Crtd_DateTime", DbType.DateTime,clsCommon.GetValueDBNull(Me.mvarCrtd_DateTime), ParameterDirection.Input,16 ))
			pc.Add(New ParamStruct("@LUpd_DateTime", DbType.DateTime,clsCommon.GetValueDBNull(Me.mvarLUpd_DateTime), ParameterDirection.Input,16 ))
			DAL.ExecPreparedSQL(PP_SMS_SalesOrdWrk, CommandType.StoredProcedure, pc,"")
		Me.mvarBranchID = clsCommon.GetValue(pc.Item("@BranchID").Value, mvarBranchID.GetType().FullName)
		Return (Me.mvarBranchID <> String.Empty )
		Catch ex As Exception
			Throw ex 
		End Try
	End Function
	Public Function Update() as Boolean 
		Dim DAL As DataAccess = m_Dal
		Try
			Dim pc As New ParamCollection
			pc.Add(New ParamStruct("@Action", DbType.String, "Update", ParameterDirection.Input,50 ))
			 pc.Add(New ParamStruct("@BranchID",DbType.String, clsCommon.GetValueDBNull(me.mvarBranchID), ParameterDirection.Input,30 ))
			 pc.Add(New ParamStruct("@OrderNbr",DbType.String, clsCommon.GetValueDBNull(me.mvarOrderNbr), ParameterDirection.Input,15 ))
			 pc.Add(New ParamStruct("@OrderDate",DbType.DateTime, clsCommon.GetValueDBNull(me.mvarOrderDate.Date), ParameterDirection.Input,16 ))
			 pc.Add(New ParamStruct("@CustID",DbType.String, clsCommon.GetValueDBNull(me.mvarCustID), ParameterDirection.Input,50 ))
			 pc.Add(New ParamStruct("@OrdAmt",DbType.Decimal, clsCommon.GetValueDBNull(me.mvarOrdAmt), ParameterDirection.Input,8 ))
			 pc.Add(New ParamStruct("@OrdStatus",DbType.Byte, clsCommon.GetValueDBNull(me.mvarOrdStatus), ParameterDirection.Input,1 ))
			 pc.Add(New ParamStruct("@SlsperId",DbType.String, clsCommon.GetValueDBNull(me.mvarSlsperId), ParameterDirection.Input,15 ))
			 pc.Add(New ParamStruct("@PicFileName",DbType.String, clsCommon.GetValueDBNull(me.mvarPicFileName), ParameterDirection.Input,100 ))
			 pc.Add(New ParamStruct("@PicFileCrtdDateTime",DbType.DateTime, clsCommon.GetValueDBNull(me.mvarPicFileCrtdDateTime), ParameterDirection.Input,16 ))
			 pc.Add(New ParamStruct("@Crtd_DateTime",DbType.DateTime, clsCommon.GetValueDBNull(me.mvarCrtd_DateTime), ParameterDirection.Input,16 ))
			 pc.Add(New ParamStruct("@LUpd_DateTime",DbType.DateTime, clsCommon.GetValueDBNull(me.mvarLUpd_DateTime), ParameterDirection.Input,16 ))
			Return (DAL.ExecNonQuery(PP_SMS_SalesOrdWrk, CommandType.StoredProcedure, pc,"") > 0)
		Catch ex As Exception
			Throw ex 
		End Try
	End Function
	Public Function Delete() as Boolean 
		Dim DAL As DataAccess = m_Dal
		Try
			Dim pc As New ParamCollection
			pc.Add(New ParamStruct("@Action", DbType.String, "Delete", ParameterDirection.Input,50 ))
			Return (DAL.ExecNonQuery(PP_SMS_SalesOrdWrk, CommandType.StoredProcedure, pc,"") > 0)
		Catch ex As Exception
			Throw ex 
		End Try
	End Function
	Public Function GetAll() as DataTable 
		Dim DAL As DataAccess = m_Dal
		Try
			Dim pc As New ParamCollection
			Dim ds As New DataSet
			pc.Add(New ParamStruct("@Action", DbType.String, "GetListData", ParameterDirection.Input,50 ))
			ds = DAL.ExecDataSet(PP_SMS_SalesOrdWrk, CommandType.StoredProcedure, pc,"")
			Return ds.Tables(0)
		Catch ex As Exception
			Throw ex 
		End Try
	End Function
	Public Sub Reset()
		mvarBranchID = String.Empty
		mvarOrderNbr = String.Empty
		mvarOrderDate = Today
		mvarCustID = String.Empty
		mvarOrdAmt = 0
		mvarOrdStatus = 0
		mvarSlsperId = String.Empty
		mvarPicFileName = String.Empty
		mvarPicFileCrtdDateTime = Today
		mvarCrtd_DateTime = Today
		mvarLUpd_DateTime = Today
	End Sub
	Public Function GetByKey() as Boolean 
		Dim DAL As DataAccess = m_Dal
		Dim ds As New DataSet 
		Try
			Dim pc As New ParamCollection
			pc.Add(New ParamStruct("@Action", DbType.String, "GetData_ByKey", ParameterDirection.Input,50 ))
			ds = DAL.ExecDataSet(PP_SMS_SalesOrdWrk, CommandType.StoredProcedure, pc,"")
			me.Reset()
			If ds Is Nothing Then
				Return False
			End If
			If ds.Tables(0).Rows.Count > 0 Then
				FillData(ds.Tables(0).Rows(0))
				Return True
			End If
		Catch ex As Exception
			Throw ex 
		End Try
		Return False
	End Function
	Public Sub FillData(row as DataRow)
		mvarBranchID =  clsCommon.GetValue(row("BranchID"), mvarBranchID.GetType().FullName)
		mvarOrderNbr =  clsCommon.GetValue(row("OrderNbr"), mvarOrderNbr.GetType().FullName)
		mvarOrderDate =  clsCommon.GetValue(row("OrderDate"), mvarOrderDate.GetType().FullName)
		mvarCustID =  clsCommon.GetValue(row("CustID"), mvarCustID.GetType().FullName)
		mvarOrdAmt =  clsCommon.GetValue(row("OrdAmt"), mvarOrdAmt.GetType().FullName)
		mvarOrdStatus =  clsCommon.GetValue(row("OrdStatus"), mvarOrdStatus.GetType().FullName)
		mvarSlsperId =  clsCommon.GetValue(row("SlsperId"), mvarSlsperId.GetType().FullName)
		mvarPicFileName =  clsCommon.GetValue(row("PicFileName"), mvarPicFileName.GetType().FullName)
		mvarPicFileCrtdDateTime =  clsCommon.GetValue(row("PicFileCrtdDateTime"), mvarPicFileCrtdDateTime.GetType().FullName)
		mvarCrtd_DateTime =  clsCommon.GetValue(row("Crtd_DateTime"), mvarCrtd_DateTime.GetType().FullName)
		mvarLUpd_DateTime =  clsCommon.GetValue(row("LUpd_DateTime"), mvarLUpd_DateTime.GetType().FullName)
	End Sub
#End Region 

#Region "Private Methods"
#End Region 

End Class
