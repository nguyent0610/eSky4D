...
var grdOrdDet_BeforeEdit = function (item, e) {
    ...
    if (record.invt && record.invt.ClassID == 'POSM' && (key == 'FreeItem' || key == 'DiscPct' || key == 'SlsPrice' || key == 'SlsUnit' || key == 'DiscAmt' || key == 'ManuDiscAmt')) return false;
...
var grdLot_BeforeEdit = function (item, e) {
    ...
    if (Ext.isEmpty(record.data.InvtID)) {
        ...
        record.data.InvtMult = (HQ.objType.INDocType == "IN" || HQ.objType.INDocType == "II") ? -1 : 1;
    }
...
var save = function () {
    ...
    App.stoOrdDet.data.each(function (item) {
        if (!Ext.isEmpty(item.data.InvtID)) {
            ...
            if (!item.data.FreeItem && item.data.BOType != "R" && item.data.LineAmt == 0 && item.data.QtyBO == 0 && Ext.isEmpty(item.data.POSM)) {
                HQ.message.show(703, '', '', true);
                flat = item;
                return false;
            }
