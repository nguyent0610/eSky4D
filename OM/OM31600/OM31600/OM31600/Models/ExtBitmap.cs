...
namespace OM31600.Models
{
    public static class ExtBitmap
    {
        ...
        public static string ToBase64Thumbnails(this string filePath, int width, int height, bool wrapImageTag)
        {
            string base64Thumbnails = string.Empty;

            if (System.IO.File.Exists(filePath))
            {
                StreamReader streamReader = new
                            StreamReader(filePath);
                System.Drawing.Image img = System.Drawing.Image.FromStream(streamReader.BaseStream);
                streamReader.Close();

                base64Thumbnails = img.ToBase64Thumbnail(width, height, wrapImageTag);

                img.Dispose();
            }
            return base64Thumbnails;
        }
        public static string ToBase64ThumbnailsHttp(this string filePath, int width, int height, bool wrapImageTag)
        {
            string imgTag = string.Empty;


            imgTag = "<img src=" + filePath;
