...
using IN10800.Models;
namespace IN10800.Controllers
{
    [DirectController]
    [CustomAuthorize]
    [CheckSessionOut]
    public class IN10800Controller : Controller
    {
        private string _screenNbr = "IN10800";
        IN10800Entities _db = Util.CreateObjectContext<IN10800Entities>(false);
        ...
        {
            return this.Store(_db.IN10800_phStockOutlet(Current.UserName, Current.CpnyID, branchID, slsperID, custID, stockType, stkOutDate).ToList());
        }

        public ActionResult GetStockOutletDet(string branchID, string slsperID, string stkOutNbr)
        {
            return this.Store(_db.IN10800_pgStockOutletDet(Current.UserName, Current.CpnyID, branchID, slsperID, stkOutNbr).ToList());
        }
        ...
        public ActionResult SaveData(FormCollection data, bool isNew)
        {
            try
            {
                ...
                var inputStockOutlet = lstStockOutletHandler.ObjectData<IN10800_phStockOutlet_Result>().FirstOrDefault();

                var lstStockOutletDetChangeHandler = new StoreDataHandler(data["lstStockOutletDetChange"]);
                var lstStockOutletDetChange = lstStockOutletDetChangeHandler.BatchObjectData<IN10800_pgStockOutletDet_Result>();

                var lstStockOutletPOSMHandle = new StoreDataHandler(data["lstStockOutletPOSM"]);
                var lstStockOutletPOSM = lstStockOutletPOSMHandle.BatchObjectData<IN10800_pgStockOutletDetPOSM_Result>();
                ...
                if (outlet != null)
                {
                    ...
                    if (isNew)
                    {
                        ...
                        if (setup != null)
                        {
                            var newStkOut = _db.IN10800_ppStkOutNbr(Current.CpnyID, Current.UserName, inputStockOutlet.BranchID).FirstOrDefault();
                            if (newStkOut != null)
                            {
        ...
        private void updateStockOutletDet(ref PPC_StockOutletDet updatedDetail, IN10800_pgStockOutletDet_Result updated, bool isNew)
        {
            if (isNew) 
            {
        ...
        private void updateStockOutletPOSM(ref PPC_StockOutletPOSM updatedDetail, IN10800_pgStockOutletDetPOSM_Result updated, PPC_StockOutlet objHeader, bool isNew)
        {
            if (isNew)
            {
        ...
        private void updateStockOutlet(ref PPC_StockOutlet outlet, IN10800_phStockOutlet_Result inputStockOutlet, bool isNew)
        {
            if (isNew) {
            ...
            try
            {
                if (fileInfo.Extension.ToLower() == ".xls" || fileInfo.Extension.ToLower() == ".xlsx")
                {
                    ...
                    for (int i = dataRowIdx; i <= workSheet.Cells.MaxDataRow; i++) //index luôn đi từ 0
                    {
                        ...
                        var custID = workSheet.Cells[i, colTexts.IndexOf("IN10800CustID")].StringValue.Trim();
                        var invtType = workSheet.Cells[i, colTexts.IndexOf("IN10800InvtType")].StringValue.Trim();
                        var stockType = workSheet.Cells[i, colTexts.IndexOf("IN10800StockType")].StringValue.Trim();
                        ...
                        try
                        {
                            if (workSheet.Cells[i, colTexts.IndexOf("IN10800StkDate")].StringValue.Length > 0)
                            {
                                stkDate = DateTime.ParseExact(workSheet.Cells[i, colTexts.IndexOf("IN10800StkDate")].StringValue, "dd/MM/yyyy",
                                       System.Globalization.CultureInfo.InvariantCulture);
                                //workSheet.Cells[i, colTexts.IndexOf("IN10800OutDate")].DateTimeValue;
                            }
                            else
                            {
                        ...
                        if (workSheet.Cells[i, colTexts.IndexOf("IN10800OutDate")].StringValue.Trim().Length == 0 && GetCodeFromExcel(stockType) != "SP")
                        {
                            stkExpDateNull += (i + 1) + ", ";
                            flagCheck = true;
                        }
                        ...
                        // stkDate = GetCodeFromExcel(stockType) == "SP" ? "1990/1/1".ToDateTime() : workSheet.Cells[i, colTexts.IndexOf("IN10800StkDate")].DateTimeValue;
                        ...
                        var posm = workSheet.Cells[i, colTexts.IndexOf("IN10800POSM")].StringValue.Trim();
                        #endregion
                        #region validate
                        ...
                        {
                            var objSlser = _db.IN10800_piSlsperID(Current.UserName, Current.CpnyID, Current.LangID, branchID, slsperID).FirstOrDefault();
                            if (objSlser == null)
                            {
                                slsperErr += (i + 1) + ", ";
                                flagCheck = true;
                        ...
                        {
                            var objCust = _db.IN10800_piCustID(Current.UserName, Current.CpnyID, Current.LangID, branchID, custID).FirstOrDefault();
                            if (objCust == null)
                            {
                                custErr += (i + 1) + ", ";
                                flagCheck = true;
                        ...
                        {
                            var objInvt = _db.IN10800_piInvtID(Current.UserName, Current.CpnyID, Current.LangID, GetCodeFromExcel(invtType), invtID).FirstOrDefault();
                            if (objInvt == null)
                            {
                                invtErr += (i + 1) + ", ";
                                flagCheck = true;
                    ...
                    message += posmNull == "" ? "" : string.Format(Message.GetString("2019022560", null), Util.GetLang("IN10800POSM"), posmNull.TrimEnd(','));
                                ...
                                using (SqlTransaction sqlTran = dbConnection.BeginTransaction())
                                {
                                    using (SqlBulkCopy s = new SqlBulkCopy(dbConnection, SqlBulkCopyOptions.KeepIdentity, sqlTran))
                                    {
                                        try
                                        {
                                            ...
                                            SqlCommand cmd1 = new SqlCommand("IN10800_ppServerImport", dbConnection, sqlTran);
        ...
        public ActionResult Export(FormCollection data)
        {
            ...
            sheet.Name = Util.GetLang("IN10800NameSheet");
            ...
            DataTable dtBranch = dal.ExecDataTable("IN10800_pcBranchExcel", CommandType.StoredProcedure, ref pc);
            MasterData.Cells.ImportDataTable(dtBranch, true, 0, 0, false);


            DataTable dtSlsper = dal.ExecDataTable("IN10800_pcSlsperExcel", CommandType.StoredProcedure, ref pc);
            MasterData.Cells.ImportDataTable(dtSlsper, true, 0, 5, false);

            DataTable dtCust = dal.ExecDataTable("IN10800_pcCustExcel", CommandType.StoredProcedure, ref pc);
            MasterData.Cells.ImportDataTable(dtCust, true, 0, 10, false);

            DataTable dtInvtType = dal.ExecDataTable("IN10800_pcInvtTypeExcel", CommandType.StoredProcedure, ref pc);
            MasterData.Cells.ImportDataTable(dtInvtType, true, 0, 15, false);

            DataTable dtStockType = dal.ExecDataTable("IN10800_pcStockTypeExcel", CommandType.StoredProcedure, ref pc);
            MasterData.Cells.ImportDataTable(dtStockType, true, 0, 20, false);


            DataTable dtInvt = dal.ExecDataTable("IN10800_pcInvtExcel", CommandType.StoredProcedure, ref pc);
            MasterData.Cells.ImportDataTable(dtInvt, true, 0, 25, false);

            DataTable dtReason = dal.ExecDataTable("IN10800_pcReasonExcel", CommandType.StoredProcedure, ref pc);
            MasterData.Cells.ImportDataTable(dtReason, true, 0, 30, false);

            DataTable dtPosm = dal.ExecDataTable("IN10800_pcPosmExcel", CommandType.StoredProcedure, ref pc);
            MasterData.Cells.ImportDataTable(dtPosm, true, 0, 35, false);

            DataTable dtDate = dal.ExecDataTable("IN10800_pcInvtDateExcel", CommandType.StoredProcedure, ref pc);
            ...
            validation.AddArea(GetCellArea(1, dtCust.Rows.Count + 100, ColTexts.IndexOf("IN10800CustID")));
            ...
            validation.AddArea(GetCellArea(1, dtInvtType.Rows.Count + 100, ColTexts.IndexOf("IN10800InvtType")));
            ...
            validation.AddArea(GetCellArea(1, dtStockType.Rows.Count + 100, ColTexts.IndexOf("IN10800StockType")));
            ...
            validation.AddArea(GetCellArea(1, dtDate.Rows.Count + 100, ColTexts.IndexOf("IN10800OutDate")));
            ...
            validation.AddArea(GetCellArea(1, dtPosm.Rows.Count + 100, ColTexts.IndexOf("IN10800POSM")));
            #endregion
            ...
            range = sheet.Cells.CreateRange(Getcell(allColumns.IndexOf("IN10800CustID")) + 2, Getcell(allColumns.IndexOf("IN10800CustID")) + 1000);
            range.SetStyle(style);

            range = sheet.Cells.CreateRange(Getcell(allColumns.IndexOf("IN10800InvtType")) + 2, Getcell(allColumns.IndexOf("IN10800InvtType")) + 1000);
            range.SetStyle(style);

            range = sheet.Cells.CreateRange(Getcell(allColumns.IndexOf("InvtID")) + 2, Getcell(allColumns.IndexOf("InvtID")) + 1000);
            range.SetStyle(style);

            range = sheet.Cells.CreateRange(Getcell(allColumns.IndexOf("IN10800StockType")) + 2, Getcell(allColumns.IndexOf("IN10800StockType")) + 1000);
            range.SetStyle(style);
            ...
            range = sheet.Cells.CreateRange(Getcell(allColumns.IndexOf("IN10800POSM")) + 2, Getcell(allColumns.IndexOf("IN10800POSM")) + 1000);
            range.SetStyle(style);

            range = sheet.Cells.CreateRange(Getcell(allColumns.IndexOf("IN10800OutDate")) + 2, Getcell(allColumns.IndexOf("IN10800OutDate")) + 1000);
            range.SetStyle(style);
            ...
            range = sheet.Cells.CreateRange(Getcell(allColumns.IndexOf("IN10800StkDate")) + 2, Getcell(allColumns.IndexOf("IN10800StkDate")) + 1000);
            range.SetStyle(style);
            ...
            {
                FileDownloadName = string.Format("{0}.xlsx", Util.GetLang("IN10800_excel"))
            };
