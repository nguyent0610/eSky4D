                    ...
                    if (this.result.data.lstTrans != undefined) {
                        ...
                        this.result.data.lstTrans.forEach(function (item) {
                            ...
                            if (!objTrans) {
                                ...
                                newTrans.set('LotSerTrack', item.LotSerTrack);
                            }
                            else {
                        ...
                        this.result.data.lstLot.forEach(function (item) {                            
                            var objTrans = HQ.store.findRecord(App.stoTrans, ['InvtID', 'SiteID', 'WhseLoc'], [item.InvtID, item.SiteID, item.WhseLoc]);
                            var objLot = HQ.store.findRecord(App.stoLotTrans, ['InvtID', 'LotSerNbr', 'INTranLineRef'], [item.InvtID, item.LotSerNbr, objTrans.data.LineRef]);
                            if (!objLot) {
                                ...
                                if (date > new Date(parseInt(item.ExpDate.substr(6)))) {
                                    newLot.set('ExpDate', date);
                                }
                                else {
                                ...
                                if (date > new Date(parseInt(item.WarrantyDate.substr(6)))) {
                                    newLot.set('WarrantyDate', date);
                                }
                                else {
                                ...
                                newLot.set('PackageID', item.PackageID);
                            }
                            else {
...
var btnLotOK_Click = function () {
    if (!App.grdLot.isLock) {
        ...
        var lstData = App.stoLotTrans.data;
        var lsterro = "";
        var lsterorPackage = "";
        if (lstData != undefined) {
            for (var i = 0; i < lstData.length; i++) {
                ...
                if (!Ext.isEmpty(lstData.items[i].data.LotSerNbr) && App.winLot.record.data.LotSerTrack == _lotQ && Ext.isEmpty(lstData.items[i].data.PackageID)) {
                    lsterorPackage = lsterorPackage + (i + 1).toString() + ",";
                }
            }
        }

        if (lsterro != "") {
            HQ.message.show(2018052211, [lsterro, HQ.common.getLang('IN10100WarrantyDate'), HQ.common.getLang('ExpDate')], '', true);
            return false;
        }

        if (lsterorPackage != "") {
            HQ.message.show(2018083011, [lsterorPackage, HQ.common.getLang('PackageID')], '', true);
            return false;
        }
    ...
    if (selected.length > 0) {
        if (!Ext.isEmpty(selected[0].data.InvtID)) {
            ...
            App.stoItemSite.load({
                params: { siteID: App.SiteID.getValue(), invtID: selected[0].data.InvtID, whseLoc: App.cboWhseLoc.getValue() },
    ...
    if (Object.keys(e.record.modified).length > 0) {
        ...
        if (!Ext.isEmpty(invt)) {
            ...
            if (key == 'InvtID') {
                ...
                App.stoItemSite.load({
                    params: { siteID: App.SiteID.getValue(), invtID: e.record.data.InvtID, whseLoc: App.cboWhseLoc.getValue() }, callback: checkSourceEdit, row: e
                });
                App.stoPrice.load({
                ...
                HQ.maxTrans = 2;
                App.stoItemSite.load({
                    params: { siteID: App.SiteID.getValue(), invtID: e.record.data.InvtID, whseLoc: App.cboWhseLoc.getValue() },
                    callback: checkSourceEdit,
                    row: e
                });
                App.stoPrice.load({
...
var grdLot_Edit = function (item, e) {
    ...
    if (Object.keys(e.record.modified).length > 0) {
        if (key == "Qty" || key == "UnitDesc") {
            ...
            var pctExpDate = 0;
            if (HQ.currentDet.data.WarrantyDays != 0) {
                pctExpDate = HQ.util.mathRound(Ext.isEmpty(e.record.data.WarrantyDate) ? 0 : 100 * (HQ.currentDet.data.WarrantyDays - inDay(e.record.data.WarrantyDate, e.record.data.ExpDate)) / HQ.currentDet.data.WarrantyDays, 2);
            }            
...
var save = function () {
        ...
        if (!Ext.isEmpty(item.data.InvtID)) {
            ...
            if (!Ext.isEmpty(item.invt) && item.invt.LotSerTrack != "N" && !Ext.isEmpty(item.invt.LotSerTrack)) {
                ...
                if (item.invt.LotSerTrack == _lotQ) {
                    detQty = item.data.UnitMultDiv == "M" ? item.data.Qty * item.data.CnvFact : item.data.Qty / item.data.CnvFact;
                } else {
                    detQty = Math.round(item.data.UnitMultDiv == "M" ? item.data.Qty * item.data.CnvFact : item.data.Qty / item.data.CnvFact);
                }
    ...
    if (line != "") {
        HQ.message.show(2018082812, line);
        flat = true;
        return false;
    }
